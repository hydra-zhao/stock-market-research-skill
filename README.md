...
> **这是一份求职作品集展示版**，展示 AI Agent / Skill 工程、金融数据工程、
> 系统架构与软件工程实践。生产环境中对应的策略实现（选股逻辑、评分权重、
> 阈值、买卖触发条件、私有 prompt）**不在本仓库公开**，相关位置以
> interface + mock/示例策略替代（见 [mock 边界声明](#mock-边界声明)）。
>
> 仅供学习与技术交流，**不构成任何投资建议**。
...
归一化、确定性渲染到**可校验交付**的完整闭环。生产版每日实盘运行，
公开版保留全部工程骨架并离线可跑。

解决的核心问题：

1. **LLM 输出不可信** → 关键事实全部由确定性管道产出，Agent 只允许引用
   canonical artifact，禁止自行拼装数字（跨 Agent 交付契约 FINAL-001）；
2. **数据源单点故障** → capability 路由 + 多源降级链 + 同族/异源血缘审计；
3. **风控纪律靠人守不住** → fail-closed 硬闸全链路化：数据缺失/核验不过 =
   显式降级（"未验证"），绝不静默假数据；
4. **策略与工程耦合** → 决策层以 Protocol 与管道解耦，可整体替换（本仓库
   即用示例策略替换了私有实现而管道零改动）。
